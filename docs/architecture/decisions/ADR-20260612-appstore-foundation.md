...
| Paid apps/IAP | sdkwork-catalog + sdkwork-order + sdkwork-payment | `commerce_product_id` references a Catalog product; clients resolve its SKU and create Order checkout through the Cloudrouter facade |
...
flowchart TB
  ...
  subgraph deps [Dependency platforms]
    ...
    CMRC[Catalog / Order / Payment via Cloudrouter]
  end
