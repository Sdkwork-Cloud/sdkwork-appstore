...
const baselinePath = path.join(
  root,
  'database/ddl/baseline/postgres/0001_appstore_legacy_baseline.sql',
);
...
const prefixRegistry = {
  ...
  prefixes: [{ prefix: 'appstore_', owner: 'appstore-platform', domain: 'appstore' }],
};
...
const schemaYaml = [
  ...
  'owner_team: appstore-platform',
  'compliance_level: L2',
  'engines:',
  '  - postgres',
  'table_prefix: appstore_',
  'tables:',
  ...tableNames.map(
    (name) => `  - name: ${name}\n    lifecycle_status: active\n    owner: appstore-platform`,
  ),
...
process.stdout.write(`materialized ${tableNames.length} tables into appstore database contract\n`);
