...
function apiServerCrate() {
  const name = "sdkwork-api-appstore-standalone-gateway";
  return {
    name,
    files: {
      [`crates/${name}/Cargo.toml`]: `${cargoToml(name)}
[[bin]]
name = "sdkwork-api-appstore-standalone-gateway"
path = "src/main.rs"
`,
...
`,
    ...
    println!("{}", sdkwork_api_appstore_standalone_gateway::server::planned_listener_name());
}
`,
...
pub fn planned_listener_name() -> &'static str {
    "TODO(appstore-implementation): start sdkwork-api-appstore-standalone-gateway listener"
}
`,
...
4. TODO(appstore-implementation): mount app-api route crates in \`sdkwork-api-appstore-standalone-gateway\`.
