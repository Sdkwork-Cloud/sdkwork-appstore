...
- SDK clients are constructed in `src/services/storeClient.ts` (app store), `src/services/driveClient.ts` (drive uploads), `src/services/commentsClient.ts` (listing reviews), and `src/services/notificationClient.ts` (inbox) and injected via hooks
