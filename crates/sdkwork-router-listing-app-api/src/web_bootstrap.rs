...
pub fn appstore_app_api_public_path_prefixes() -> Vec<String> {
    vec!["/healthz".to_owned()]
}

pub fn appstore_app_api_prefixes() -> Vec<String> {
    vec![PREFIX.to_owned()]
}
...
pub fn wrap_router_with_web_framework(
    ...
    route_manifest
        .validate_public_path_prefixes(&appstore_app_api_public_path_prefixes())
        .expect("appstore app-api public prefixes must not cover protected manifest routes");

    let layer = WebFrameworkLayer::new(resolver)
        .with_profile(WebRequestContextProfile {
            public_path_prefixes: appstore_app_api_public_path_prefixes(),
